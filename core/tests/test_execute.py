...
@patch.dict(
    os.environ, {"QSTASH_CURRENT_SIGNING_KEY": "c", "QSTASH_NEXT_SIGNING_KEY": "n"}
)
async def test_execute_recovery_success_and_masks_data(mock_receiver):
    """Return success and mask payment data after sending a recovery email.

    Args:
        mock_receiver: Mocked QStash signature verifier.
    """
    ...
    db_payload = {
        "payload": {"data": {"customer_id": "test@fucina.com", "cart_id": "c1"}},
        "discount_pct": 0.15,
    }
...
async def test_execute_recovery_not_found(mock_receiver):
    """Skip recovery when no scheduled recovery record is found.
